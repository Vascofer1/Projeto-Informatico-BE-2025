...
try {
  ...
  // Se o user colar um JSON válido como string
  if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
    ...
    // Caso contrário, assume que o utilizador escreveu uma por linha
...
try {
  ...
  // fallback de segurança (array vazio)
  options = []
}
  ...
  try {
    ...
    formSelections[response.data.id] = { included: false, mandatory: false }
...
<template>
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl max-w-md mx-auto">
      <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">Suggested Questions</h2>

      <div v-for="question in questions" :key="question.id"
        class="bg-white dark:bg-gray-900 rounded-lg shadow-md p-4 mb-3 flex justify-between items-center">
        <div>
          <h3
  class="text-sm font-semibold text-gray-800 dark:text-gray-100 cursor-pointer hover:underline"
...
>
  {{ question.name }}
          ...
          <div v-if="formSelections[question.id]?.included" class="mt-1 text-xs">
            <p v-if="formSelections[question.id]?.mandatory" class="text-red-500 dark:text-red-400 flex items-center">
              <span class="ml-2">Answer is required</span>
            </p>
            <p v-else class="text-gray-500 dark:text-gray-400 flex items-center">
              <span class="ml-2">Answer is optional</span>
            </p>
          </div>
        </div>
        ...
        <div class="flex flex-col items-end gap-1">
          ...
          <label v-if="formSelections[question.id].included"
  ...
  class="bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600 transition flex items-center gap-2"
>
  <Plus class="w-4 h-4" /> Create New Question
</button>
      </div>

      <!-- Card de criação de pergunta -->
  ...
  <div v-if="selectedQuestion" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    <div class="bg-white dark:bg-gray-900 w-full max-w-md p-6 rounded-2xl shadow-xl relative">
      ...
      <h2 class="mb-4 text-3xl font-black text-gray-700 dark:text-gray-200">
      ...
      <button class="mt-4 w-full bg-blue-500 text-white py-2 rounded hover:bg-blue-600 dark:bg-blue-700"
        @click="submitForm">
        Confirm
      </button>
    </div>
