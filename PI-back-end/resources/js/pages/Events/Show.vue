    ...
    <div class="p-6 bg-gray-100 dark:bg-gray-900 min-h-screen space-y-8">
      ...
      <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 flex-1 flex gap-6"
          ...
          <!-- Imagem -->
          <div v-if="event.image" class="w-full md:w-48 h-auto aspect-[2/3] rounded-xl overflow-hidden shadow-lg mt-14">
            <img :src="`/storage/${event.image}`" alt="Event Image" class="w-full h-full object-cover" />
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div
            class="bg-green-50 dark:bg-green-900 border-l-4 border-green-500 p-4 rounded-xl flex items-center space-x-4 shadow-sm">
          ...
          <div
            class="bg-yellow-50 dark:bg-yellow-900 border-l-4 border-yellow-500 p-4 rounded-xl flex items-center space-x-4 shadow-sm">
        ...
        <div v-if="chartData.length > 0" class="mt-10 space-y-6">
          <div class="flex flex-wrap gap-4 mb-4">
            <label v-for="(chart, index) in chartData" :key="`checkbox-${index}`"
              class="inline-flex items-center space-x-2 text-sm text-gray-800 dark:text-gray-200">
              <input type="checkbox" v-model="shownCharts[index]" class="form-checkbox h-4 w-4 text-blue-600" />
